...
def _move_static(obj, pos_ch, space):
    ...
    if obj.type == 'Poly':
        ...
        obj._cpSensor.collision_type = COLTYPE_GOAL
...
def noisifyWorld(gameworld, noise_position_static = 0., noise_position_moving = 5.,
                 noise_collision_direction = .2, noise_collision_elasticity = .2, noise_gravity = .1,
                 noise_object_friction = .1, noise_object_density = .1, noise_object_elasticity = .1, per_object_noise = {}):

    # static moving - noise in the initial position of static / moving objs
    # collision direction and elasticity - direction and elasticity of the ball after it comes off of the object
    w = gameworld.copy()
    ...
    if noise_position_static > 0:
        ...
        for og in obj_groups:
            ...
            for o in og:
                if o.isStatic():
                    # override aggregate noise with object specific one
                    obj_noise = per_object_noise.get(o.name, None)
                    if obj_noise:
                        pos_change = obj_noise * norm.rvs(size = 2)
    ...
    if noise_position_moving > 0:
        ...
        while len(free_obj) > 0 and noise_attempts < max_attempts:
            ...
            for o in free_obj:
                try:
                    # _n = norm(0, 1)
                    shift = noise_position_moving*norm.rvs(size = 2)
                    # shift = noise_position_moving*_n.rvs(size = 2)
                    o.position = (o.position[0] + shift[0], o.position[1] + shift[1])
    ...
    if noise_position_moving > 0:
        def noisifyTeleport(arb, space, data):
            # print("called")
            o1, _ = arb.shapes
            obj = w.objects[o1.name]
            x_change = noise_position_moving * norm.rvs()
            obj._cpBody.position += [x_change.item(), 0]
            return True

        tsch = w._cpSpace.add_collision_handler(COLTYPE_SOLID, COLTYPE_TELEPORTER)
        tsch.separate = noisifyTeleport
    ...
    def doSolidSensorBegin(arb, space, data):
        return w._checkSensors(arb, space, data)
    ...
    ssench = w._cpSpace.add_collision_handler(COLTYPE_SOLID, COLTYPE_GOAL)
    ssench.begin = doSolidGoalBegin
    ssench.separate = doSolidGoalEnd

    psench = w._cpSpace.add_collision_handler(COLTYPE_PLACED, COLTYPE_GOAL)
    psench.begin = doSolidGoalBegin
    psench.separate = doSolidGoalEnd

    sensorch = w._cpSpace.add_collision_handler(COLTYPE_SOLID, COLTYPE_SENSOR)
    sensorch.begin = doSolidSensorBegin
    psensorch = w._cpSpace.add_collision_handler(COLTYPE_PLACED, COLTYPE_SENSOR)
