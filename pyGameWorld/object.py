...
import pdb
import copy
__all__ = ['PGPoly','PGBall','PGSeg','PGContainer','PGCompound','PGGoal',
           'PGBlocker','PGSensor','PGTeleporter']

class PGObject(object):

    def __init__(self, name, otype, space, color, density, friction, elasticity):
        assert otype in ['Ball','Poly','Segment','Container', 'Compound','Goal','Blocker','Sensor','Teleporter'], \
...
class PGPoly(PGObject):
    ...
    def contains(self, point):
        # very crude - just use the bounding rectangle to determine if the point is within
        # the polygon area
        ...
        if np.all(point >= p1) and np.all(point <= p2):
            return True
...
class PGBall(PGObject):
    ...
    area = property(getArea)
    position = property(getPos, PGObject.setPos)
...
class PGContainer(PGObject):
    ...
    def __init__(self,name, space, ptlist, width, density = DEFAULT_DENSITY,
        ...
        self._cpSensor.collision_type = COLTYPE_GOAL
    ...
    def pointIn(self, p):
        v = pm.Vec2d(p[0], p[1])
        pq = self._cpSensor.point_query(v)
        return (pq.distance <= 0.)
...
class PGGoal(PGObject):
    ...
    def __init__(self, name, space, vertices, color):
        ...
        self._cpShape.collision_type = COLTYPE_GOAL
        self._cpShape.name = name
        space.add(self._cpShape)
    ...
    def pointIn(self,p):
        v = pm.Vec2d(p[0], p[1])
        pq = self._cpShape.point_query(v)
        return (pq[0] <= 0.)

    vertices = property(getVertices)

class PGSensor(PGGoal):

    def __init__(self, name, space, vertices, color=DEFAULT_COLOR):
        PGObject.__init__(self, name, "Sensor", space, color, 0, 0, 0)
        self._cpShape = pm.Poly(space.static_body, vertices)
        self._cpShape.sensor = True
...
class PGTeleporter(PGGoal):

    # def __init__(self, name, space, vertices, exit_coord, color=DEFAULT_COLOR):
    def __init__(self, name, space, center, radius, exit_coord, is_entry, color=DEFAULT_COLOR):
        PGObject.__init__(self, name, "Teleporter", space, color, 0, 0, 0)
        # self._cpShape = pm.Poly(space.static_body, vertices)
        self.radius = radius
        self.center = center
        self._cpShape = pm.Circle(space.static_body, radius, center)
        self._cpShape.sensor = True
        self._cpShape.collision_type = COLTYPE_SENSOR
        self._cpShape.name = name
        ...
        if is_entry:
            # add an inner circle that actually teleports
            self._teleporterShape = pm.Circle(space.static_body, 5.0, center)
            self._teleporterShape.sensor = True
            self._teleporterShape.collision_type = COLTYPE_TELEPORTER
            self._teleporterShape.name = name
            space.add(self._teleporterShape)
