...
__all__ = ['areaForSegment','areaForPoly','centroidForPoly','recenterPoly','objectComplexity',
           'segs2Poly','polyValidate', 'word2Color', 'distanceToObject', 'calculateDerivatives','objectBoundingBox',
           'calculateDerivativeNextAction', 'filterCollisionEvents', 'lineToPointDist', 'followTheGradient',
...
def segs2Poly(seglist, r):
    ...
    for i in range(1, len(vlist)-1):
        ...
        if angn < 0:
            angn += 2*np.pi
        unitn = pm.Vec2d(0, 1).rotated(angn)
        # unitn.angle = angn
...
def calculateDerivatives(func, f_xy, x, y, h, k,tp,hessian=False, return_vals=False):
...
def followTheGradient(func, centered_obj, action, h=1, k=1, num_samples=1, num_steps=5, step_size=5, return_vals=False):
    ...
    for step in range(num_steps):
        ...
        for i in range(num_samples):
            derivs = calculateDerivatives(func, centered_obj, next_step[0], next_step[1], h, k, tp, True, return_vals)
            if len(derivs) > 7:
...
def calculateDerivativeNextAction(func, centered_obj, action, h=1, k=1, num_samples=1):
    x = action[0]
    y = action[1]
    deriv_samps = np.zeros((1,5))
    #run the derivatives calculation for a given number of samples
    ...
    for i in range(num_samples):
        deriv_samps = np.vstack((deriv_samps, calculateDerivatives(func, centered_obj, x, y, h, k, False)))
